...
namespace HomagConnect.MmrMobile.Samples.Console.AlertsEvents
{
    /// <summary>
    ...
    internal static class AlertEventsSamples
    {
        ...
        {
            var machines = (await mmrMobileClient.GetMachines()).ToList();
            if (machines.Any())
            {
                foreach (var machine in machines)
                {
                    var alerts = (await mmrMobileClient.GetAlertEventsFromMachine(machine.MachineNumber ?? "", DateTime.Now.AddDays(-14), DateTime.Now, 1000)).ToList();
                    if (alerts.Any())
                    {
