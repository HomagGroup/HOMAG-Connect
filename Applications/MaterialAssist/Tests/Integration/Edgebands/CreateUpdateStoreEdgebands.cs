...
[TestClass]
[IntegrationTest("MaterialAssist")]
[IntegrationTest("MaterialAssist.Edgebands")]
public class CreateUpdateStoreEdgebands : MaterialAssistTestBase
{
    ...
    public async Task Edgebands_CreateUpdateStore()
    {
        ...
        var random = Guid.NewGuid().ToString()[..10];

        var edgebandCodeAbsMultiplex = "ABS_Multiplex_schwarz_1.00_23.0_NN" + random;
        var edgebandCodeNnSchwarz = "NN_Schwarz_16.50_24.5_HM" + random;
        var edgebandCodeAbsDashOfFreedom = "ABS_A_Dash_of_Freedom_1.00_100.0_HM" + random;

        try
        {
            await EnsureEdgebandTypeExist(edgebandCodeAbsMultiplex, 1, 225);
            await EnsureEdgebandTypeExist(edgebandCodeNnSchwarz, 1, 75);
            await EnsureEdgebandTypeExist(edgebandCodeAbsDashOfFreedom, 16.5, 50);

            // edgeband data
            var edgeband1 = new { id = Guid.NewGuid().ToString(), edgebandCode = edgebandCodeAbsMultiplex };
            var edgeband2 = new { id = Guid.NewGuid().ToString(), edgebandCode = edgebandCodeNnSchwarz };
            var edgeband3 = new { id = Guid.NewGuid().ToString(), edgebandCode = edgebandCodeAbsDashOfFreedom };
            ...
            {
                EdgebandCode = edgebandCodeAbsMultiplex,
                Thickness = 1.2,
            };
            await materialManagerClient.UpdateEdgebandType(edgeband1.edgebandCode, edgebandTypeUpdate);
            var updatedEdgeband1 = new { id = "87", edgebandCode = edgebandCodeAbsMultiplex };
        ...
        {
            await materialManagerClient.DeleteEdgebandType(edgebandCodeAbsMultiplex);
            await materialManagerClient.DeleteEdgebandType(edgebandCodeNnSchwarz);
            await materialManagerClient.DeleteEdgebandType(edgebandCodeAbsDashOfFreedom);
        }
    }
}