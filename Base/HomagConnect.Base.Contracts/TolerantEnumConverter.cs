...
{
    /// <summary>
    /// We use this converter in order to extend in the future enum entries without breaking the deserialization of old assembly versions
    ...
    public class TolerantEnumConverter : JsonConverter
    {
          ...
          JsonSerializer serializer)
        {
            ...
            if (flag)
                return (object)null;
            string str1 = ((IEnumerable<string>)names).FirstOrDefault<string>((Func<string, bool>)(n => string.Equals(n, "Unknown", StringComparison.OrdinalIgnoreCase))) ?? ((IEnumerable<string>)names).First<string>();
